...
while read -r line; do
    ...
    if [[ "$line" =~ Info/Join.*\ ([^@]+)@([^: ]+):\ Joined\ session ]]; then
        ...
        if [ -z "${user_counts["$sid"]}" ]; then 
            ...
            echo "[Monitor] Bot command: xvfb-run -a $CMD_PATH --join $SERVER_URL/$sid --headless --username $BOT_NAME"
            xvfb-run -a "$CMD_PATH" --join "$SERVER_URL/$sid" --headless --username "$BOT_NAME" 2>&1 | sed 's/^/[BOT] /' &
            bot_pids["$sid"]=$!
