...
CMD sh -c "./sync-to-appwrite.sh restore && (while true; do sleep 10 && ./sync-to-appwrite.sh backup; done) & BACKUP_PID=\$!  && ./squashfs-root/usr/bin/drawpile-srv --database /home/drawpile/data/drawpile. db --sessions /home/drawpile/data/sessions --listen 0.0.0.0 --port 27750 --websocket-listen 0.0.0.0 --websocket-port ${PORT:-10000} --allow-guest-hosts true 2>&1 | ./monitor-sessions.sh; kill \$BACKUP_PID"
